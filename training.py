...
def t_epoch(model, load, optimizer, epoch, epochs, trainnum = None):
    ...
    for batch_indx, (global_feats, local_feats, labels, nucle_idx, rna_id) in enumerate(load): ### nucle_idx: sample id
        ...
        loss = torch.nn.functional.binary_cross_entropy(output.squeeze(), label_vert).cuda()
...
def train(model, train_dataset, test19, save=None, batch_size=32, train_number=6, epochs=30, train_files=None,
    ...
    nucleo_samples = len(rna_list)  # 训练集的样本总数
    split_id = int(splite_rate * nucleo_samples)  # 训练集划分点
    valid_id = nucleo_samples - split_id  # 验证集样本数
    np.random.shuffle(rna_list)
    train_idx = rna_list[:split_id]
    valid_idx = rna_list[split_id:]
    train_samples = sampler.SubsetRandomSampler(train_idx)  # 创建训练集、验证集和测试集的数据采样器
    valid_samples = sampler.SubsetRandomSampler(valid_idx)  # 验证集
    ## load test set
    test19_samples = sampler.SubsetRandomSampler(test19_list)  # 测试集
    ## load dataset
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=train_samples,
                                               pin_memory=(torch.cuda.is_available()), num_workers=6,
                                               drop_last=False)  # 创建训练集、验证集和测试集的数据加载器
    ...
    ## multi GPU
    model_wrapper = model
    optimizer = torch.optim.Adam(model_wrapper.parameters(), lr=0.0001)  # 创建优化器，使用 Adam 优化算法，学习率为 0.0001
    best_p_max = 0.0
    best_r_max = 0.0
    best_auc = 0.0
    for epoch in range(epochs):  ## a epoch of train and valid\
        ...
        if auc > best_auc:
            best_auc = auc
            threadhold = t_max
            torch.save(model.state_dict(), os.path.join(save, './model5.dat'))
